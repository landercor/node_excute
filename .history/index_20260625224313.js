...
app.post('/register', async (req, res) => {
    ...
    try { // los metodos y propiedades estaticas podemo utilizarlas sin necesidad de crear una instancia dela clase.
        ...
        res.send({ id }) //devolver el id

    } catch (error) {
        console.log(error)
        res.status(400)
        .send({
            error: error.message //siempre doble error:error
        }) // normalmente no se debe enviar el error del repositorio.
    }
